<template>
   <q-layout>
     ...
     <q-card class="q-pa-md">
       <q-tabs v-model="selectedTab" dense>
         <q-tab name="addData" label="Add Data" />
         <q-tab name="reports" label="Reports" />
         <q-tab name="profileSettings" label="Profile Settings" />
       </q-tabs>
 
       <q-tab-panels v-model="selectedTab">
         <q-tab-panel name="addData">
          <div class="q-pa-md row items-start q-gutter-md">
  <div class="q-col-12 q-sm-6 q-md-6 q-lg-6">
  ...
  <div class="flex gap-5 bg-gray-200 rounded p-2 w-full">
    <div @click="toggleDiv" class="text-blue-700 flex items-center cursor-pointer">
    <q-icon :name="isDivVisible ? 'arrow_upward' : 'arrow_downward'" size="25px" />
    <span class="ml-1 cursor-pointer"><strong>{{isDivVisible ? 'Close' : 'Add Data'}}</strong></span>
  </div>

  <div @click="toggleDialog" class="text-blue-700 flex items-center cursor-pointer">
    <q-icon name="post_add" size="25px" />
    <span class="ml-1 cursor-pointer"><strong>Dispatch to Provinces</strong></span>
  </div>
  </div>
  ...
  <storeSupply
   ...
   :dialogVisibility="dialog" 
    @close="handleClose"
/>
  <div v-if="isDivVisible" class="container mx-auto">

    
  <table class="min-w-full table-auto border-collapse">
    <thead>
      <!-- You can add table headers here -->
    </thead>
    <tbody>
      <!-- Row 1: Product and Shop Registration -->
      <tr>
        <td class="border border-green-500 rounded-md p-6 text-center align-bottom">
          <div class="w-full max-w-md">
            <div class="q-gutter-md mb-4">
              <div class="text-h6 mb-2">Add Product to Main Store</div>
              <select v-model="selectedProductCode" @change="handleProductChange"
                class="bg-gray-50 border border-gray-300 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-180 p-2.5">
        ...
        <td class="border border-green-500 rounded-md p-4 text-center align-bottom">
          <div class=" bg-gray-300 p-4 m-4">
            <div class="w-full max-w-md">
            <div class="q-gutter-md mb-4">
              <div class="text-h6 mb-2">Edit or Delete from Main store</div>
              <div class="">
                <select v-model="selectedProductCode" @change="handleProductChange"
        ...
        <td class="border border-green-500 rounded-md p-4 text-center align-bottom">
          <div class="text-h6 mb-2">Register New Department</div>
        ...
        <td class="border border-green-500 rounded-md p-4 text-center align-bottom">
          <div>
            <div class="q-gutter-md" style="max-width: 300px">
              <div class="text-h6 mb-2">Register New User</div>
              <form  @submit.prevent="handleSubmit">
                <q-input v-model="email" type="email" label="email" />
              <q-input v-model="password" type="password" label="eassword" />
              ...
              <select v-model="department2" id="countries" class="bg-gray-50 border border-gray-300 text-gray-900 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 font-bold">
  ...
  <div class="mt-2">
    <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Register</button>
        ...
        <td class="border border-green-500 rounded-md p-4 text-center align-bottom">
          <div>
            <div class="text-h6 mb-2">Register New Shop</div>
...
<div class="flex justify-center gap-4 mt-4">
  ...
  <div class="text-h6 mb-2">Upload Data to Database</div>
          <div>
            <input type="file" @change="handleFileUpload" class="mb-2" />
            
              
              <q-btn @click="importData" flat style="color: #FF0080" label="import to firebase" />
          </div>
</div>
<table>
      <thead>
      ...
      <input
        ...
        class="block p-2 text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500"
      />
      <button  @click="getDispatchLogs"  class="bg-blue-500 text-white p-2 rounded-lg">Search</button>
    </div>
    ...
    <table class="styled-table">
      <thead>
        <tr class="cursor-pointer">
          ...
          <th  @click="sortTable('departmentName')">Province</th>
          <th>Status</th>
          <th @click="sortTable('productName')">Product</th>
          <th>Quantity</th>
          <th>Actions</th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="(dispatch, index) in sortedLogs" :key="index">
          <td>{{ formatDateTime(dispatch.date) }}</td>
          <td>{{ getDepartmentName(dispatch.department) }}</td>
        <td>{{ dispatch.Status }}</td>
          <td>{{ getProductName(dispatch.productCode) }}</td>
          <td>{{ dispatch.dispatchedAmount }}</td>
          <td><button @click="handleDelete(dispatch.department, dispatch.productCode, dispatch.dispatchedAmount,dispatch.id)">Delete</button></td>
        </tr>
      </tbody>
    </table>
...
<p v-if="message" class="success-message">{{ message }}</p>
         ...
         <q-tab-panel name="profileSettings">
           <div>
             <h3>Profile Settings</h3>
             <q-form @submit="updateProfile">
               <q-input v-model="userProfile.name" label="Name" />
               <q-input v-model="userProfile.email" type="email" label="Email" />
